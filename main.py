...
base_stake       = 5.00
current_stake    = 5.00
take_profit_total = 100.00
stop_loss_total  = 50.00
loss_streak      = 0
max_loss_streak  = 4
total_staked     = 0.0
total_payout     = 0.0
...
def predict_digits(recent_digits, n=3):
    """
    Smart 4-signal prediction engine:
    1. Frequency — most common digits
    2. Gap — digits not seen recently
    3. Pattern — what follows current digit
    4. Anti-streak — avoid repeating digits
    """
    ...
    if next_digits:
        ...
        pattern_top    = [
            d for d, _ in pattern_counts.most_common(4)
        ]
...
def get_confidence(recent_digits, digits):
    ...
    if len(digits) >= 2:
        base_conf = min(base_conf * 1.2, 95)
...
def dashboard():
    ...
    for item in trade_history:
        row_color = (
            "lightgreen" if "Won:0" not in item
            else "#ff6b6b"
        )
        history_rows += (
            "<p style='color:" + row_color +
            "'>" + item + "</p>"
        )
    ...
    total_per_round = round(base_stake * PREDICTIONS, 2)
    win_one_profit  = round((base_stake * 8) - (base_stake * (PREDICTIONS - 1)), 2)
    win_two_profit  = round((base_stake * 16) - (base_stake * (PREDICTIONS - 2)), 2)
    win_all_profit  = round(base_stake * 8 * PREDICTIONS - base_stake * PREDICTIONS, 2)

    page = (
        "<!DOCTYPE html><html><head>"
        "<title>DIGITMATCH PRO ENGINE</title>"
        "<style>"
        ...
        "<p>Last Digit: <b style='font-size:32px;color:gold'>" + str(last_digit) + "</b></p>"
        "<p>" + signal + "</p>"
        "</div>"

        "<div class='card'>"
        "<h3>" + str(PREDICTIONS) + " Digit Predictions</h3>"
        + digit_boxes +
        "<p style='font-size:13px;color:#94a3b8;margin-top:10px'>"
        "Stake $" + str(base_stake) + " x " + str(PREDICTIONS) +
        " = $" + str(total_per_round) + " per round"
        "</p>"
        "<p style='font-size:13px;color:lightgreen'>"
        "Win 1 = +$" + str(win_one_profit) +
        " | Win 2 = +$" + str(win_two_profit) +
        "</p>"
        "</div>"
        ...
        "<p>Predictions (1-5)</p>"
        "<input type='number' name='preds' value='" + str(PREDICTIONS) + "'>"
        "<p>Take Profit ($)</p>"
