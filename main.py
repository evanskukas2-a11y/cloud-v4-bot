...
def analyse_digits(recent_digits):
    ...
    if avg_jump <= 2.5:
        return True, 90, f"CALM MARKET | Avg jump: {avg_jump:.1f}"

    if avg_jump <= 3.5:
        return True, 80, f"MODERATE MARKET | Avg jump: {avg_jump:.1f}"

    return False, 60, f"BORDERLINE | Avg jump: {avg_jump:.1f}"
...
<head>
    ...
    <script>
        // Smart refresh — stops when typing in inputs
        let refreshTimer;
