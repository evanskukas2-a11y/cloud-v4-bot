...
if not TOKEN:
    print("ERROR: DERIV_TOKEN missing")
...
def analyse_digits(recent_digits):
    ...
    if avg_jump <= 2.5:
        return True, 90, (
            f"CALM MARKET | Avg jump: {avg_jump:.1f}"
        )

    if avg_jump <= 3.5:
        return True, 80, (
            f"MODERATE MARKET | Avg jump: {avg_jump:.1f}"
        )

    return False, 60, (
        f"BORDERLINE | Avg jump: {avg_jump:.1f}"
    )
...
def update_settings(
    ...
    global base_stake
    global current_stake
    global take_profit_per_trade
    global stop_loss_per_trade
    global take_profit_total
    global stop_loss_total
    global GROWTH_RATE
    global TARGET_TICKS
